    ...
    if (workerCount > 0) {
      ...
      if (hasEnhanced) {
        ...
        if (entryCount > 0) {
          ...
          const assistantMessages = page.locator('[data-testid="log-entry"][data-type="assistant_message"]');
          ...
          const structuredEntries = await userMessages.count() + 
  ...
  test('should display appropriate icons for different log types', async ({ page }) => {
    ...
    if (workerCount > 0) {
      ...
      if (await enhancedBadge.isVisible()) {
        ...
        if (foundIcons === 0) {
          console.warn('⚠️ No log entry icons found - enhanced logging may not be working properly');
        }
      }
    }
  });
