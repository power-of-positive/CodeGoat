...
describe('Basic Referential Integrity Validation', () => {
  ...
  test('should maintain referential integrity during complex operations', async () => {
    ...
    const project = await apiClient.projects.create({
      name: `Data Consistency Test Project ${timestamp}`,
      git_repo_path: `/tmp/data-consistency-test-${timestamp}`,
  ...
  test('should prevent orphaned records during transaction failures', async () => {
    const timestamp = Date.now();
    const project = await apiClient.projects.create({
      name: `Transaction Test Project ${timestamp}`,
      git_repo_path: `/tmp/transaction-test-${timestamp}`,
      use_existing_repo: false
    });
