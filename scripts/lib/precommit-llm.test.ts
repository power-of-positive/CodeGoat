...
describe('precommit-llm', () => {
  ...
  describe('runLlmReviewProcess', () => {
    it('should return null when SKIP_LLM_REVIEW is set to true', async () => {
      ...
      expect(vi.mocked(llmReviewGenerator.generateLlmReviewComments)).not.toHaveBeenCalled();

      consoleSpy.mockRestore();
    });

    it('should return null when LLM review passes', async () => {
      vi.mocked(llmReviewGenerator.generateLlmReviewComments).mockResolvedValue(undefined);
      ...
      expect(result).toBeNull();
      expect(vi.mocked(llmReviewGenerator.generateLlmReviewComments)).toHaveBeenCalledWith(
        '/mock/project'
      );
    ...
    it('should handle LLM review generation errors gracefully', async () => {
      ...
      vi.mocked(llmReviewGenerator.generateLlmReviewComments).mockRejectedValue(error);
    ...
    it('should handle LLM check errors gracefully', async () => {
      vi.mocked(llmReviewGenerator.generateLlmReviewComments).mockResolvedValue(undefined);
      vi.mocked(llmCheck.checkLlmReview).mockRejectedValue(new Error('Check failed'));
      const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    it('should handle non-Error exceptions', async () => {
      vi.mocked(llmReviewGenerator.generateLlmReviewComments).mockRejectedValue('String error');
      const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    it('should not be skipped when SKIP_LLM_REVIEW is set to other values', async () => {
      ...
      expect(result).toBeNull();
      expect(vi.mocked(llmReviewGenerator.generateLlmReviewComments)).toHaveBeenCalled();
    });
  });
