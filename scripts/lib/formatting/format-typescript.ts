 ...
 */
function createTempTsConfig(projectRoot: string, stagedFiles: string[]): string {
 ...
 */
function executeTypeScriptCheck(projectRoot: string, tempConfigPath: string): CheckResult {
  try {
    const result = execCommand('npx tsc --noEmit --project tsconfig.staged.json', projectRoot);
