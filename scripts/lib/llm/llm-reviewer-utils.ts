...
} from "./llm-reviewer-types";

/**
 * Generate formatted report from review results
 */
export function generateReport(
  reviews: Array<{ file: string; result: ReviewResult }>,
): string {
...
export function generateStructuredData(
  reviews: ReviewedFile[],
): StructuredReviewData {
  const highSeverity = reviews.filter(
    (r) => r.result.severity === "high",
  ).length;
  const mediumSeverity = reviews.filter(
    (r) => r.result.severity === "medium",
  ).length;
  const totalIssues = reviews.reduce(
    (sum, r) => sum + r.result.issues.length,
    0,
