 ...
 */
export function createErrorResult(error: unknown, context?: string): StandardError {
  const standardError = standardizeError(error);

  return {
    ...standardError,
    message: context ? `${context}: ${standardError.message}` : standardError.message,
  };
}
...
export function logError(error: unknown, context?: string): void {
  ...
  console.error(`❌ Error${context ? ` (${context})` : ''}: ${standardError.message}`);
