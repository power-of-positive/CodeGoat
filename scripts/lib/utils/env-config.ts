 ...
 */
export async function loadProjectEnv(scriptsDepth: number = 2): Promise<EnvLoadResult> {
  try {
...
export function loadProjectEnvSync(scriptsDepth: number = 2): EnvLoadResult {
  try {
    ...
    if (fs.existsSync(envPath)) {
      ...
      const variablesLoaded = result.parsed ? Object.keys(result.parsed).length : 0;
