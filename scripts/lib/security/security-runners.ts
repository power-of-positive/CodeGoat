...
import { validateCommand, validateDirectoryExists } from '../utils/validation-utils';
 ...
 */
export function runDuplicateCodeDetection(projectRoot: string): SecurityCheckResult {
  try {
    ...
    return createSuccessResult(`🔍 Duplicate code check completed (informational only)`);
  } catch {
...
function handleDeadCodeError(output: string): SecurityCheckResult {
  ...
  if (
    ...
    return createSuccessResult(`⚠️  Dead code check: Environmental issue, skipping check`);
  }

  if (output.includes('unused') || (output.trim().length > 0 && !output.includes('No issues'))) {
...
export function runDeadCodeDetection(projectRoot: string): SecurityCheckResult {
  try {
    ...
    runSecurityCommand(projectRoot, 'npx unimported --init false --show-preset-config false');
...
function handleVulnerabilityError(output: string): SecurityCheckResult {
  if (output.includes('vulnerabilities') && !output.includes('found 0 vulnerabilities')) {
    return createFailureResult(`🚨 DEPENDENCY VULNERABILITIES:\n${output}`);
  }
  return createSuccessResult(`⚠️  Dependency check: ${output.trim() || 'Completed'}`);
}

/**
 * Run dependency vulnerability checks using npm audit
 */
export function runDependencyVulnerabilityCheck(projectRoot: string): SecurityCheckResult {
  try {
    const result = runSecurityCommand(projectRoot, 'npm audit --audit-level moderate --omit dev');
