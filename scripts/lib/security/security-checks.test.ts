...
const mockRunDuplicateCodeDetection = runDuplicateCodeDetection as jest.MockedFunction<typeof runDuplicateCodeDetection>;
const mockRunDeadCodeDetection = runDeadCodeDetection as jest.MockedFunction<typeof runDeadCodeDetection>;
const mockRunDependencyVulnerabilityCheck = runDependencyVulnerabilityCheck as jest.MockedFunction<typeof runDependencyVulnerabilityCheck>;
...
describe('security-checks', () => {
  ...
  describe('runSecurityChecks', () => {
    ...
    it('should pass all security checks when all succeed', () => {
      ...
      expect(result.securityOutput).toContain('Dependency Vulnerabilities: No vulnerabilities found');
    ...
    it('should report failure when duplicate code detection fails', () => {
      ...
      expect(result.securityOutput).toContain('Duplicate Code Detection: Duplicate code found in files X, Y');
    ...
    it('should report failure when dependency vulnerability check fails', () => {
      ...
      expect(result.securityFailure).toBe(true);
      expect(result.securityOutput).toContain('Dependency Vulnerabilities: 3 vulnerabilities found');
    ...
    it('should handle Error exceptions gracefully', () => {
      ...
      expect(result.securityFailure).toBe(false);
      expect(result.securityOutput).toContain('Duplicate Code Detection: ⚠️ Failed to run - Network connection failed');
      
      expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️ Duplicate Code Detection check failed: Network connection failed');
      expect(consoleSpy).toHaveBeenCalledWith('✅ Dead Code Detection passed');
      expect(consoleSpy).toHaveBeenCalledWith('✅ Dependency Vulnerabilities passed');
    });
    ...
    it('should handle non-Error exceptions gracefully', () => {
      ...
      expect(result.securityFailure).toBe(false);
      expect(result.securityOutput).toContain('Duplicate Code Detection: ⚠️ Failed to run - String error message');
      expect(result.securityOutput).toContain('Dead Code Detection: ⚠️ Failed to run - [object Object]');
      expect(result.securityOutput).toContain('Dependency Vulnerabilities: ⚠️ Failed to run - null');
      
      expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️ Duplicate Code Detection check failed: String error message');
      expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️ Dead Code Detection check failed: [object Object]');
      expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️ Dependency Vulnerabilities check failed: null');
    });
    ...
    it('should handle mixed success, failure, and error scenarios', () => {
      ...
      expect(result.securityOutput).toContain('Dead Code Detection: ⚠️ Failed to run - Process crashed');
      expect(result.securityOutput).toContain('Dependency Vulnerabilities: All dependencies secure');
      
      expect(consoleSpy).toHaveBeenCalledWith('❌ Duplicate Code Detection failed');
      expect(consoleWarnSpy).toHaveBeenCalledWith('⚠️ Dead Code Detection check failed: Process crashed');
      expect(consoleSpy).toHaveBeenCalledWith('✅ Dependency Vulnerabilities passed');
    });
