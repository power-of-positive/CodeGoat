 ...
 */
export function analyzeStructuredReview(reviewData: StructuredReviewData): string {
  const severityIssues: string[] = [];

  // Check for high severity issues
  const highSeverityFiles = reviewData.files.filter(f => f.result.severity === 'high');
  if (highSeverityFiles.length > 0) {
    severityIssues.push(`HIGH: ${highSeverityFiles.length} file(s) with critical issues detected`);
    highSeverityFiles.forEach(file => {
      if (file.result.issues.length > 0) {
        severityIssues.push(`  - ${file.file}: ${file.result.issues.join(', ')}`);
      }
    });
  }

  // Check for medium severity issues
  const mediumSeverityFiles = reviewData.files.filter(f => f.result.severity === 'medium');
  ...
  const blockingFiles = reviewData.files.filter(f => f.result.hasBlockingIssues);
