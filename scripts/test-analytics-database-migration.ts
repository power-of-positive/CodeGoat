...
async function testAnalyticsWithDatabase() {
  ...
  try {
    ...
    console.log(
      `✅ Analytics loaded: ${analytics.totalSessions} sessions, ${analytics.totalValidationRuns} validation runs`
    );

    console.log('\n📈 Testing getStageStatistics()...');
    const stageStats = await analyticsService.getStageStatistics('lint');
    console.log(
      `✅ Stage stats for 'lint': ${stageStats.overview.totalAttempts} attempts, ${stageStats.overview.successRate.toFixed(1)}% success rate`
    );
    ...
    console.log(
      `✅ Run statistics: ${runStats.totalRuns} total runs, ${runStats.successRate.toFixed(1)}% success rate`
    );
