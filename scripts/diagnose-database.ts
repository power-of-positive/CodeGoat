...
async function diagnoseDatabasePath() {
  ...
  try {
    ...
    if (taskCount > 0) {
      ...
      sampleTasks.forEach((task, index) => {
        ...
        console.log(
          `${index + 1}. [${taskId}] ${task.content.substring(0, 60)}... (${task.status})`
        );
      });
  ...
  dbFiles.forEach((file: string) => {
    ...
    console.log(
      `📄 ${file}: ${Math.round(stats.size / 1024)}KB, modified ${stats.mtime.toISOString()}`
    );
  });
}
...
if (require.main === module) {
  diagnoseDatabasePath().catch(error => {
