...
async function detectFrontendPort() {
  ...
  for (const port of commonPorts) {
    try {
      ...
      console.log(`🔍 Detected frontend server running on port ${port}`);
      return port;
    } catch {
  ...
  console.log('⚠️ No frontend server detected on common ports, using default 5173');
  return 5173;
}
...
async function runTests() {
  ...
  console.log(`📡 Using frontend port: ${detectedPort}`);
