...
function formatExportsList(exports: UnusedExport[], maxDisplay: number = 15): string {
  ...
  const byFile = exports.reduce((acc, exp) => {
  ...
  const result = files.map(file => {
    ...
    const exportsList = fileExports.slice(0, 5).map(e => e.export).join(', ');
...
function runDeadCodeDetection(): void {
  ...
  try {
    ...
    // Filter exports based on ignore patterns
    const filteredExports = allExports.filter(exp => 
      !shouldIgnoreFile(exp.file, config.ignorePatterns)
    );
    
    const unusedCount = filteredExports.length;
    console.log(`📊 Unused exports after filtering: ${unusedCount} (max: ${config.maxUnusedExports})`);
