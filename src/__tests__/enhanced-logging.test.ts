...
describe('Enhanced Logging', () => {
  ...
  describe('Enhanced Log Endpoint Logic', () => {
    ...
    it('should not mix raw JSON with normalized entries', () => {
      ...
      const rawPatches = patches.filter(
        p => p.value.type === 'STDOUT' || p.value.type === 'STDERR'
      );
  ...
  describe('inferActionType method', () => {
    ...
    it('should infer file_read action for file read tools', () => {
      const fileReadAction = (processor as any).inferActionType('file_read_tool', {
        path: '/test/file.ts',
      });
    ...
    it('should infer file_edit action for write tools', () => {
      const writeAction = (processor as any).inferActionType('Write', {
        file_path: '/test/file.ts',
      });
    ...
    it('should infer web_fetch action for web tools', () => {
      const webAction = (processor as any).inferActionType('WebFetch', {
