...
jest.mock('./HistoricalTimeline', () => ({
  HistoricalTimeline: function MockHistoricalTimeline({
    stageId,
    dateRange,
    autoRefresh,
    refreshInterval,
...
jest.mock('./PerformanceComparison', () => ({
  PerformanceComparison: function MockPerformanceComparison({
    stageId,
    dateRange,
    environment,
  }: any) {
...
jest.mock('../../../shared/ui/select', () => ({
  SimpleSelect: ({ children, ...props }: any) => (
    <select data-testid={props['data-testid'] || 'select'} {...props}>
      {children}
    </select>
  ),
...
const renderWithRouter = (component: React.ReactElement, initialUrl = '/') => {
  return render(<MemoryRouter initialEntries={[initialUrl]}>{component}</MemoryRouter>);
};
...
describe('StageHistoryDashboard', () => {
  ...
  describe('Rendering and Initial State', () => {
    it('should render with default props', () => {
      ...
      expect(
        screen.getByText('Detailed performance metrics and reliability analysis')
      ).toBeInTheDocument();
    });
