...
function StageLogs({ stage, hasError, hasOutput }: {
  stage: ValidationStageResult;
  hasError: boolean;
  hasOutput: boolean;
}) {
...
function StageDetailExpanded({ stage }: { stage: ValidationStageResult }) {
  ...
  return (
    <Card className="mb-4">
      ...
      {showLogs && hasLogs && (
        <StageLogs stage={stage} hasError={hasError} hasOutput={hasOutput} />
      )}
    </Card>
  );
}
...
function useAllTasks(config: {
  ...
  return useQuery<TaskWithValidationRuns[]>({
    ...
    enabled: !config.dbLoading && !config.dbRun && !config.dbError && !config.analyticsLoading && !config.analyticsRuns?.find((r) => r.id === config.runId),
  });
}
...
function useValidationRunData(runId: string | undefined) {
  ...
  const { data: analyticsRuns, isLoading: analyticsLoading, error: analyticsError } = useAnalyticsRuns(dbLoading, dbRun, dbError);
...
function NotFoundState({ runId, navigate }: { runId: string | undefined; navigate: (path: string) => void }) {
  return (
    <div className="flex items-center justify-center min-h-96">
      <div className="text-center">
...
// Header component
function ValidationRunHeader({ run, taskContext, navigate }: {
  run: ValidationRun;
