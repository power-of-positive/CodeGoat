...
const getEntryIcon = (entryType: NormalizedEntryType) => {
  ...
  if (entryType.type === 'tool_use') {
    ...
    if (actionType?.action === 'file_write') {
    ...
    if (actionType?.action === 'search') {
      return <Search className="h-4 w-4 text-indigo-600" />;
    }
    if (actionType?.action === 'web_fetch') {
...
function DisplayConversationEntry({ entry, index }: Props) {
  ...
  return (
    <div key={index} className="px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800/50">
          ...
          {isErrorMessage && hasMultipleLines ? (
            <div className={isExpanded ? 'space-y-2' : ''}>
              <div className={getContentClassName(entry.entry_type)}>
                {isExpanded ? (
                  renderContent(entry.content, entry.entry_type)
                ) : (
                  <>
                    {entry.content.split('\n')[0]}
                    <button
