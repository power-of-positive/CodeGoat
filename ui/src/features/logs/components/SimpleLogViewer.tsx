...
export default function SimpleLogViewer({
  ...
  const processedLogs = entries.flatMap(entry => {
    ...
    if (entry.channel === 'stdout' || entry.channel === 'stderr') {
      ...
      // Handle assistant responses
        ...
        typeof payload.entry_type === 'object' &&
        payload.entry_type !== null &&
        'type' in payload.entry_type &&
        (payload.entry_type as Record<string, unknown>).type === 'assistant_message' &&
      ...
      // Handle tool use
        ...
        payload.entry_type !== null &&
        'type' in payload.entry_type &&
        (payload.entry_type as Record<string, unknown>).type === 'tool_use' &&
        typeof payload.content === 'string'
      ) {
  ...
  const formatLogLine = (
    logLine: { content: string; channel: 'stdout' | 'stderr' },
    index: number
  ) => {
    return (
      <RawLogText
  ...
  return (
    <div className={`h-full ${className}`}>
      {processedLogs.length === 0 ? (
        <div className="p-4 text-center text-muted-foreground text-sm">No logs available</div>
      ) : (
        <Virtuoso
          ref={virtuosoRef}
