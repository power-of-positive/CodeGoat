...
describe('Settings Component', () => {
  ...
  it('renders settings page with correct title', async () => {
    ...
    expect(
      screen.getByText(/configure validation pipeline stages/i)
    ).toBeInTheDocument();
  });
  ...
  it('displays validation stages section', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', { name: /validation stages/i })
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows add stage form when add button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole('heading', { name: /add new validation stage/i })
      ).toBeInTheDocument();
  ...
  it('can cancel the add stage form', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole('heading', { name: /add new validation stage/i })
  ...
  it('handles API errors gracefully', async () => {
    (settingsApi.getValidationStages as jest.Mock).mockRejectedValue(
      new Error('API Error')
    );
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to load validation stages/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('shows no stages message when no stages are configured', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/no validation stages configured/i)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/add your first validation stage/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('handles add stage error and closes form', async () => {
    const user = userEvent.setup();
    (settingsApi.addValidationStage as jest.Mock).mockRejectedValue(
      new Error('Add Error')
    );
  ...
  it('handles update stage error and closes form', async () => {
    const user = userEvent.setup();
    (settingsApi.updateValidationStage as jest.Mock).mockRejectedValue(
  ...
  it('can move stage up in order', async () => {
    ...
    const finalTestCall = calls.find(
      (call: any) => call[0] === 'test' && call[1].priority === 1
    );
    const finalLintCall = calls.find(
      (call: any) => call[0] === 'lint' && call[1].priority === 2
    );
