...
describe('Settings Component', () => {
  it('renders settings page with correct title', () => {
    ...
    expect(screen.getByText(/configure codegoat application settings/i)).toBeInTheDocument();
  });

  it('displays validation pipeline card with link to stage management', () => {
    renderWithProviders(<Settings />);

    expect(screen.getByRole('heading', { name: /validation pipeline/i })).toBeInTheDocument();
    expect(
  ...
  it('displays security & permissions card with link to permissions page', () => {
    ...
    expect(screen.getByRole('heading', { name: /security & permissions/i })).toBeInTheDocument();
    expect(screen.getByText(/configure executor security permissions/i)).toBeInTheDocument();
  ...
  it('displays analytics & monitoring card with link to analytics page', () => {
    ...
    expect(screen.getByRole('heading', { name: /analytics & monitoring/i })).toBeInTheDocument();
    expect(screen.getByText(/view analytics dashboards, performance metrics/i)).toBeInTheDocument();
