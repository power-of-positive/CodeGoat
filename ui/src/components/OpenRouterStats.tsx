        ...
        <div className="mt-3 pl-4 border-l-2 border-slate-100">
          ...
          {error && (
            <div className="text-sm text-red-600">
              Unable to load statistics for this model
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`font-medium ${stats.averageUptime > 95 ? 'text-green-600' : stats.averageUptime > 85 ? 'text-yellow-600' : 'text-red-600'}`}>
                    ...
                    {stats.endpoints.slice(0, 3).map((endpoint, index) => (
                      <div key={index} className="grid grid-cols-3 gap-3 text-xs">
                        <div>
                          ...
                          {endpoint.moderated && (
                            <span className="ml-1 text-slate-400">(mod)</span>
                          )}
                        </div>
                        <div className="flex items-center gap-1">
                          <Clock className="h-3 w-3 text-slate-400" />
                          <span className={endpoint.uptime > 95 ? 'text-green-600' : endpoint.uptime > 85 ? 'text-yellow-600' : 'text-red-600'}>
                            {endpoint.uptime}%
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
