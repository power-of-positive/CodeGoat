...
describe('useProcessesLogs', () => {
  ...
  it('should generate normalized conversation entries for codingagent processes', () => {
    ...
    const userMessage = normalizedEntries.find(
      e =>
        typeof e.payload === 'object' &&
        'entry_type' in e.payload &&
        e.payload.entry_type?.type === 'user_message'
    );
    const toolUse = normalizedEntries.find(
      e =>
        typeof e.payload === 'object' &&
        'entry_type' in e.payload &&
        e.payload.entry_type?.type === 'tool_use'
    );
    const assistantMessage = normalizedEntries.find(
      e =>
        typeof e.payload === 'object' &&
        'entry_type' in e.payload &&
        e.payload.entry_type?.type === 'assistant_message'
    );
  ...
  it('should update entries when processes change', () => {
    const { result, rerender } = renderHook(({ processes }) => useProcessesLogs(processes), {
