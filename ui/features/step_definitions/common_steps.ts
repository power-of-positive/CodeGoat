...
When('I select {string} from the {string} dropdown', async function (this: CustomWorld, optionText: string, dropdownLabel: string) {
  if (!this.page) throw new Error('Page not initialized');
  await this.page.getByLabel(dropdownLabel).click();
  await this.page.getByText(optionText).click();
});

When('I enter {string} in the {string} field', async function (this: CustomWorld, text: string, fieldLabel: string) {
  if (!this.page) throw new Error('Page not initialized');
...
Then('I should see the {string} section', async function (this: CustomWorld, sectionName: string) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator(`[data-testid="${sectionName.toLowerCase().replace(/\s+/g, '-')}-section"]`)).toBeVisible();
});
...
Then('I should see their current status \\({string}\\)', async function (this: CustomWorld, statuses: string) {
  if (!this.page) throw new Error('Page not initialized');
...
Then('I should see which scenarios are linked to Playwright tests', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
...
Then('I should see the overall BDD coverage percentage', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="coverage-percentage"], .coverage-metric')).toBeVisible();
});

Then('I should see total number of BDD scenarios', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="total-scenarios"], .scenarios-count')).toBeVisible();
});

Then('I should see number of scenarios with linked Playwright tests', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="linked-scenarios"], .linked-count')).toBeVisible();
});

Then('I should see number of scenarios without test coverage', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="unlinked-scenarios"], .unlinked-count')).toBeVisible();
});