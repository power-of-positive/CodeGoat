...
When('I look at the agent selector dropdown', async function (this: CustomWorld) {
  ...
  await expect(this.page.locator('[data-testid="agent-selector"], select[name*="agent"]')).toBeVisible();
});

Then('I should see {string} as an option', async function (this: CustomWorld, optionText: string) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator(`option[value="${optionText}"], option:has-text("${optionText}")`)).toBeVisible();
});

// Agent switching steps
Given('the default {string} is selected', async function (this: CustomWorld, defaultSelection: string) {
  if (!this.page) throw new Error('Page not initialized');
...
When('I select {string} from the agent dropdown', async function (this: CustomWorld, agentName: string) {
  if (!this.page) throw new Error('Page not initialized');
  const agentValue = agentName.toLowerCase().replace(/\s+/g, '_');
  await this.page.locator('select[name*="agent"], [data-testid="agent-selector"]').selectOption(agentValue);
});

Then('the analytics data should update to show only Claude CLI results', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  // Wait for data to update
  await this.page.waitForTimeout(1000);
  
  // Check that metrics cards show updated data
  await expect(this.page.locator('[data-testid="metrics-cards"], .analytics-summary')).toBeVisible();
});

Then('the summary cards should reflect Claude CLI metrics', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="summary-cards"] [data-agent="claude_cli"]')).toBeVisible();
});

Then('the charts should update with Claude CLI data points', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="analytics-charts"], .chart-container')).toBeVisible();
});

Then('the recent runs list should show only Claude CLI runs', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="recent-runs"] [data-agent="claude_cli"]')).toBeVisible();
});

// Task editing steps
Given('I have a task {string} in the pending column', async function (this: CustomWorld, taskTitle: string) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('.pending-column, [data-status="pending"]').getByText(taskTitle)).toBeVisible();
});

When('I click the edit icon on the task card', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await this.page.locator('[data-testid="edit-task"], .edit-icon, [aria-label="Edit task"]').first().click();
});

Then('a task edit modal should open', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="task-edit-modal"], .modal, [role="dialog"]')).toBeVisible();
});
...
Then('I should see fields for title, description, priority, and status', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('input[name="title"], [data-field="title"]')).toBeVisible();
  await expect(this.page.locator('textarea[name="description"], [data-field="description"]')).toBeVisible();
  await expect(this.page.locator('select[name="priority"], [data-field="priority"]')).toBeVisible();
...
When('I update the description to {string}', async function (this: CustomWorld, newDescription: string) {
  if (!this.page) throw new Error('Page not initialized');
  await this.page.locator('textarea[name="description"], [data-field="description"]').fill(newDescription);
});
...
Then('the modal should close', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="task-edit-modal"], .modal, [role="dialog"]')).not.toBeVisible();
});
...
Then('I should see a success notification', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('.success-notification, .toast-success, [data-testid="success-message"]')).toBeVisible();
});

// BDD Scenario management steps
Given('I have a task marked as type {string}', async function (this: CustomWorld, taskType: string) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator(`[data-task-type="${taskType}"], .task-type-${taskType}`)).toBeVisible();
});
...
When('I enter scenario title {string}', async function (this: CustomWorld, scenarioTitle: string) {
  if (!this.page) throw new Error('Page not initialized');
  await this.page.locator('input[name="scenarioTitle"], [data-field="scenario-title"]').fill(scenarioTitle);
});
...
When('I view the coverage by feature section', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  await expect(this.page.locator('[data-testid="coverage-by-feature"], .feature-coverage')).toBeVisible();
});

Then('I should see each feature listed with its coverage percentage', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
...
Then('I should be able to click on a feature to see its scenarios', async function (this: CustomWorld) {
  if (!this.page) throw new Error('Page not initialized');
  const firstFeature = this.page.locator('.feature-item').first();
  await firstFeature.click();
  await expect(this.page.locator('.feature-scenarios, [data-testid="feature-scenarios"]')).toBeVisible();
});