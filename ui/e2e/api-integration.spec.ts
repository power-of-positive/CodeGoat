...
test.describe('API Integration Tests', () => {
  test('API endpoints are accessible and working', async ({ request }) => {
    ...
    const modelsResponse = await request.get('http://localhost:3001/api/models');
  ...
  test('test endpoint validates model existence', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:3001/api/models/test/999');
  ...
  test('test endpoint returns consistent response structure', async ({ request }) => {
    ...
    for (const modelId of modelIds) {
      const response = await request.post(`http://localhost:3001/api/models/test/${modelId}`);
  ...
  test('model CRUD operations work correctly', async ({ request }) => {
    // Test add model
    const addResponse = await request.post('http://localhost:3001/api/models', {
      data: {
    ...
    const deleteResponse = await request.delete(`http://localhost:3001/api/models/${newModelId}`);
  ...
  test('models endpoint returns real model data from config', async ({ request }) => {
    const response = await request.get('http://localhost:3001/api/models');
