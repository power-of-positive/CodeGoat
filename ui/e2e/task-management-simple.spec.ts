...
test.describe('Task Management', () => {
  ...
  test('should display task management page', async ({ page }) => {
    ...
    // Check description
    await expect(page.locator('p:has-text("Manage your tasks with a kanban-style board")')).toBeVisible();
  ...
  test('should open add task form when clicking Add Task', async ({ page }) => {
    ...
    const formElements = await page.locator('form, [role="dialog"], input, textarea, select').count();
  ...
  test('should display task cards if tasks exist', async ({ page }) => {
    ...
    const taskCards = page.locator('[class*="Card"]').filter({ hasText: /Start|Complete|To Pending/ });
