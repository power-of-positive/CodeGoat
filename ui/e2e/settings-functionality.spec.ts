...
test.describe('Settings Functionality', () => {
  ...
  test('should display fallback configuration options', async ({ page }) => {
    ...
    if ((await fallbackConfigButton.count()) > 0) {
      ...
      if ((await fallbackTriggers.count()) > 0)
        await expect(fallbackTriggers.first()).toBeVisible();
      ...
      const contextLengthFallback = page.locator(
        'label:has-text("Fallback on context length exceeded")'
      );
      if ((await contextLengthFallback.count()) > 0)
        await expect(contextLengthFallback).toBeVisible();
